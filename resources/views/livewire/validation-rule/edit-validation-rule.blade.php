                        ...
                        <div class="row mb-3 border p-3 rounded align-items-center" wire:key="{{ $index }}">
                            ...
                            @if (in_array('in', $fields[$index]['rules'] ?? []))
                                <div class="col-md-6 mt-2">
                                    <label for="in_values_{{ $index }}" class="form-label">Allowed Values
                                        (comma-separated):</label>
                                    <input type="text" id="in_values_{{ $index }}" class="form-control"
                                        wire:model="fields.{{ $index }}.in_values">
                            ...
                            @if (in_array('min', $fields[$index]['rules'] ?? []))
                                <div class="col-md-6 mt-2">
                                    <label for="min_value_{{ $index }}" class="form-label">Minimum
                                        Value:</label>
                                    <input type="number" id="min_value_{{ $index }}" class="form-control"
                                        wire:model="fields.{{ $index }}.min_value">
                            ...
                            @if (in_array('max', $fields[$index]['rules'] ?? []))
                                <div class="col-md-6 mt-2">
                                    <label for="max_value_{{ $index }}" class="form-label">Maximum
                                        Value:</label>
                                    <input type="number" id="max_value_{{ $index }}" class="form-control"
                                        wire:model="fields.{{ $index }}.max_value">
...
<div class="col-12">
    <form wire:submit.prevent="updateValidationRule">
        <div class="card">
            <div class="card-body">
                <x-alerts.alerts />
                ...
                <div>
                    <div class="mb-3">
                        ...
                        <select class="form-control" wire:model="template_id">
                            <option>--Select an option--</option>
                            @foreach ($templates as $template)
                                <option value="{{ $template->id }}" {{ $template->id == $template_id ? 'selected' : '' }}>
                                    {{ $template->name }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <label class="form-label">Add validation rules for each column of the template</label>
                    @foreach ($fields as $index => $field)
                        <div class="row mb-3 border p-3 rounded align-items-center" wire:key="{{ $field['uuid'] }}">
                            <div class="col-md-4">
                                <label for="column_name_{{ $index }}" class="form-label">Column Name:</label>
                                <input type="text" id="column_name_{{ $index }}" class="form-control"
                                    wire:model="fields.{{ $index }}.column">
                            ...
                            {{-- Prefill for "in" rule --}}
                            @if (in_array('in', $field['rules'] ?? []))
                                <div class="col-md-6 mt-2">
                                    <label for="in_values_{{ $index }}" class="form-label">Allowed Values
                                        (comma-separated):</label>
                                    <input type="text" id="in_values_{{ $index }}" class="form-control"
                                        wire:model="fields.{{ $index }}.in_values"
                                        value="{{ $field['in_values'] ?? '' }}">
                                    @error("fields.$index.in_values")
                                        <span class="text-danger">{{ $message }}</span>
                                    @enderror
                                </div>
                            ...
                            {{-- Prefill for "min" rule --}}
                            @if (in_array('min', $field['rules'] ?? []))
                                <div class="col-md-6 mt-2">
                            ...
                            {{-- Prefill for "max" rule --}}
                            @if (in_array('max', $field['rules'] ?? []))
                                <div class="col-md-6 mt-2">
